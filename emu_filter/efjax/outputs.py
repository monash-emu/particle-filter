...
def plot_particle_results(prop_particles, resamp_particles, observations):
    ...
    norm_prop_counts = prop_counts["Counts"] / prop_counts["Counts"].sum()
    norm_resamp_counts = resamp_counts["Counts"] / resamp_counts["Counts"].sum()

    results_plot = plt.scatter(
        prop_counts["Columns"] - 0.2,
        prop_counts["Values"],
        s=norm_prop_counts * 10000.0,
        alpha=0.5,
        label="proposed",
    )
    plt.scatter(
